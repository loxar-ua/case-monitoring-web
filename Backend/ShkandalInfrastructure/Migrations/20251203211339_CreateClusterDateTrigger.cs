...
{
    /// <inheritdoc />
    public partial class CreateClusterDateTrigger : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql("""
                ...
                BEGIN 
                    IF (TG_OP = 'UPDATE' OR TG_OP = 'DELETE') 
                       ...
                       AND OLD.is_checked = TRUE 
                       AND OLD.is_relevant = TRUE THEN 

                        IF (TG_OP = 'DELETE') 
                           OR (NEW.cluster_id IS DISTINCT FROM OLD.cluster_id) 
                           OR (NEW.is_checked = FALSE) 
                    ...
                    IF (TG_OP = 'INSERT' OR TG_OP = 'UPDATE') 
                       ...
                       AND NEW.is_checked = TRUE 
                       AND NEW.is_relevant = TRUE THEN 
            ...
            migrationBuilder.Sql("""
                CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_article_cluster_calc_checked_relevant
                ON article (cluster_id, published_at DESC)
                WHERE is_checked = TRUE AND is_relevant = TRUE;
        ...
        {
            migrationBuilder.Sql("DROP TRIGGER IF EXISTS tr_maintain_cluster_dates ON article;");
            migrationBuilder.Sql("DROP FUNCTION IF EXISTS sync_clusters_full_recalc;");

            migrationBuilder.Sql("DROP INDEX IF EXISTS idx_article_cluster_calc_checked_relevant;", suppressTransaction: true);
        }
    }
}
